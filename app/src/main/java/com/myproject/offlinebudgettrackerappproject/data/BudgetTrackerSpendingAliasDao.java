...
//    void insertStoreName(String date1, String date2, String storeName);
    ...
    @Query("insert into budget_tracker_spending_alias_table (date_alias, store_name_alias, product_name_alias, product_type_alias, price_alias, is_tax_alias, tax_rate_alias, alias_percentage) select date, store_name, product_name, product_type, price, is_tax, tax_rate, count(product_type) * 100.0 / (select count(*) from budget_tracker_spending_table where date >= :dateTo and date <= :dateFrom and store_name LIKE '%' || :storeName || '%') as 'alias_percentage' from budget_tracker_spending_table where date >= :dateTo and date <= :dateFrom and store_name LIKE '%' || :storeName || '%'  group by product_type;")
    void insertStoreName(String dateTo, String dateFrom, String storeName);

    @Query("insert into budget_tracker_spending_alias_table (date_alias, store_name_alias, product_name_alias, product_type_alias, price_alias, is_tax_alias, tax_rate_alias, alias_percentage) select date, store_name, product_name, product_type, price, is_tax, tax_rate, count(store_name) * 100.0 / (select count(*) from budget_tracker_spending_table where date >= :dateTo and date <= :dateFrom and product_name LIKE '%' || :productName || '%') as 'alias_percentage' from budget_tracker_spending_table where date >= :dateTo and date <= :dateFrom and product_name LIKE '%' || :productName || '%'  group by store_name;")
    void insertProductName(String dateTo, String dateFrom, String productName);

    @Query("insert into budget_tracker_spending_alias_table (date_alias, store_name_alias, product_name_alias, product_type_alias, price_alias, is_tax_alias, tax_rate_alias, alias_percentage) select date, store_name, product_name, product_type, price, is_tax, tax_rate, count(store_name) * 100.0 / (select count(*) from budget_tracker_spending_table where date >= :dateTo and date <= :dateFrom and product_type LIKE '%' || :productType || '%') as 'alias_percentage' from budget_tracker_spending_table where date >= :dateTo and date <= :dateFrom and product_type LIKE '%' || :productType || '%'  group by store_name;")
